...
public class RESTSessionCatalog extends BaseViewSessionCatalog
  ...
  public Table loadTable(SessionContext context, TableIdentifier identifier) {
    ...
    try {
      response = loadInternal(context, identifier, snapshotMode, responseHeaders::putAll);
      ...
      if (metadataType != null) {
        ...
        try {
          response = loadInternal(context, baseIdent, snapshotMode, h -> {});
    ...
    if (response == null) {
      // response can be null if the server returned a 304 - NotModified code.
      // TODO gaborkaszab: what if the table object is not present in the cache anymore?
      ...
      tableMetadata =
          TableMetadata.buildFrom(response.tableMetadata())
              ...
              .setSnapshotsSupplier(
                  () ->
                      loadInternal(context, finalIdentifier, SnapshotMode.ALL, h -> {})
                          .tableMetadata()
                          .snapshots())
              .discardChanges()
              .build();
  ...
  private class Builder implements Catalog.TableBuilder {
    ...
    public Transaction replaceTransaction() {
      ...
      LoadTableResponse response =
          loadInternal(context, ident, snapshotMode, responseHeaders::putAll);
      String fullName = fullTableName(ident);
