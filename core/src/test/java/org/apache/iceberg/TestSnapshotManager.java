...
public class TestSnapshotManager extends TableTestBase {
  ...
  @Test
  public void testSnapshotManagerWithInvalidTransactionParameters() throws Exception {
    Assert.assertThrows(
        "Input transaction cannot be null",
        NullPointerException.class,
        () -> {
          new SnapshotManager(null);
        });

    String tableName = "someTblName";
    BaseTransaction transaction =
        (BaseTransaction)
            Transactions.newTransaction(
                tableName, new TestTables.TestTableOperations(tableName, temp.newFolder()));
    Assert.assertNull(transaction.ops().current());
    Assert.assertThrows(
        "Cannot manage snapshots: table " + tableName + " does not exist",
        NullPointerException.class,
        () -> {
          new SnapshotManager(transaction);
        });
  }
}
