...
public class SparkColumnUpdateWrite extends BaseSparkWrite
  ...
  private Map<String, DataFile> writtenFiles(WriterCommitMessage[] messages) {
    ...
    for (WriterCommitMessage message : messages) {
      if (message != null) {
        ...
        Map<String, DataFile> subResult = taskCommit.updateFilesByBasePath();
        // TODO gaborkaszab: check if there is no existing entry for each base file path?
  ...
  private class ColumnUpdateOperation implements BatchWrite {
    ...
    public void commit(WriterCommitMessage[] messages, WriteSummary summary) {
      ...
      // TODO gaborkaszab: check that all data files have a new update files
      ...
      for (Map.Entry<String, DataFile> updateFile : writtenFiles.entrySet()) {
        Preconditions.checkState(
            baseFilesByPath.containsKey(updateFile.getKey()),
            "Update file references unknown base file ",
            updateFile.getKey());

        baseToUpdateFile.put(baseFilesByPath.get(updateFile.getKey()), updateFile.getValue());
      }

      // TODO gaborkaszab: conflict detection with isolationLevel similar to CoWOperation?

      ColumnUpdate columnUpdate = table.newColumnUpdate(orderedFieldIds);
  ...
  private static class ColumnUpdateDataWriter implements DataWriter<InternalRow> {
    ...
    // TODO gaborkaszab: consider creqting DataWriter here instead of RollingDataWriter
    ...
    private void closeCurrentWriter() throws IOException {
      if (currentWriter != null) {
        ...
        if(result.dataFiles().isEmpty()) {
          throw new IllegalStateException("Unable to create update file for base file "+ currentFilePath);
        }
        if(result.dataFiles().size() > 1) {
          throw new IllegalStateException("Multiple update files created for base file "+ currentFilePath);
        }
